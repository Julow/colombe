...
(** Read a line *)
type ic = unit -> string Lwt.t

(** Write a raw string *)
type oc = string -> unit Lwt.t

let main ~auth:(login, password) ~sender ~recipient ~data ic oc =
  let rec go = function
    | `Error s -> Fmt.pf Fmt.stdout "Error: %s\n%!" s; Lwt.fail Exit
    | `Read (buf, off, len, continue) ->
      ic () >>= fun s ->
      let s = s ^ "\r\n" in
      Bytes.blit_string s 0 buf off (min (String.length s) len);
      go (continue (min (String.length s) len))
    | `Write (buf, off, len, continue) ->
      let s = Bytes.sub_string buf off len in
      Fmt.pf Fmt.stdout "Sending: \x1b[31m%s\x1b[0m\n" s;
      oc s >>= fun () ->
      go (continue len)
    | `End state ->
      match state with
...
let send_mail
  ...
  in
  main ~auth ~sender ~recipient ~data
